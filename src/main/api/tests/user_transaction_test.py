...
class TestTransferMoney:
    def test_transfer_money(self, db_session: Session, api_manager: ApiManager,
        ...
        itog = deposit_amount - transfer_money.amount
        response = api_manager.transfer_steps.transfer_user(transfer_money, create_credit_request)
        assert response.fromAccountIdBalance == itog # не хочется заменять
        #хардкодным значением, может есть красивый способ избежать мат. операцию и хардкода для этого ассерта
    ...
    def test_transfer_invalid(self, api_manager: ApiManager, two_account_create: CreateCreditRequest,
                              create_credit_request: CreateCreditRequest, incorrect_transfer_amount: float):
